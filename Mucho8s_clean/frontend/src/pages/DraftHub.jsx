...
export default function DraftHub() {
  ...
  const result = mode === "auto" ? autoResult : manualResult;

  const chemistryPreview = useMemo(() => {
    const pool = mode === "auto"
      ? selectedPlayers
  ...
  const generateDraft = () => {
    ...
    const draft = draftTeamsByChemistry(selectedPlayers, matches);
    if (!draft) {
      toast.error("Unable to generate this draft");
      return;
    }

    setAutoResult(draft);
    toast.success(`Draft generated · ${draft.balanceScore}% balance`);
  };
  ...
  const createChalls = async () => {
    ...
    if (!created) return;
    toast.success(`${created.length} chall pairings created from this draft`);
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
        <div>
          ...
          <h2 className="font-display text-3xl font-extrabold">Draft Hub</h2>
          <p className="text-sm text-muted-foreground mt-1 max-w-2xl">
            Use automatic chemistry drafting or build the teams yourself and check how balanced they are.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-2 max-w-xl">
        ...
        >
          <div className="flex items-center gap-2 font-bold">
            <FlaskConical size={17} /> Auto Draft
          </div>
          <div className="text-xs mt-1">Chemistry + balance decide the teams.</div>
        </button>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-5 items-start">
        <div className="xl:col-span-5 card-surface rounded-2xl p-5">
          <div className="flex items-center justify-between gap-3 mb-4">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-display font-bold text-lg">
                {mode === "auto" ? "Draft Pool" : "Build Teams"}
              </h3>
            </div>
            <span className={`font-mono font-bold ${
              (mode === "auto" ? autoCount === required : manualCount === required)
                ? "text-emerald-400"
                : "text-[#D5A33A]"
            }`}>
              {mode === "auto" ? autoCount : manualCount}/{required}
            </span>
          </div>
          ...
          {mode === "auto" ? (
            ...
            >
              <FlaskConical size={17} className="mr-2" /> Generate Chemistry Draft
            </Button>
          ) : (
          ...
          {!result ? (
            <div className="card-surface rounded-2xl min-h-[330px] flex flex-col items-center justify-center text-center p-8">
              ...
              <h3 className="font-display text-2xl font-bold mt-4">
                {mode === "auto" ? "Draft Engine Ready" : "Build both teams"}
              </h3>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                  <div>
                    <div className="brand-kicker mb-1">
                      {mode === "auto" ? "Recommended Draft" : "Manual Team Analysis"}
                    </div>
                ...
                {isAdmin && (
                  <div className="mt-5 pt-5 border-t border-[#1D222C]">
                    ...
                    <p className="text-xs text-muted-foreground mt-2">
                      Challs are created only after this confirmation; the draft itself never sends them automatically.
                    </p>
                  </div>
                )}
              </div>
